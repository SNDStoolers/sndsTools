...
create_mock_ir_ben_r <- function(patients_ids = create_mock_patients_ids()) {
    ...
    extended_patients <- bind_rows(
        patients_ids,
        patients_ids[additional_rows, ] %>%
            mutate(BEN_NIR_PSA = BEN_NIR_PSA + 50000, BEN_RNG_GEM = 2)
    )
    ...
    extended_patients %>%
        mutate(
            ...
            BEN_NAI_ANN = sample(1950:2000, n(), replace = TRUE),
            BEN_NAI_MOI = sample(1:12, n(), replace = TRUE),
            BEN_SEX_COD = sample(c(1, 2), n(), replace = TRUE),
            BEN_ORG_AFF = sample(
                c("01", "02", "03"),
                n(),
                replace = TRUE,
...
create_mock_ir_imb_r <- function(
    ...
    patients_expanded %>%
        mutate(
            ...
            IMB_ALD_DTF = IMB_ALD_DTD + sample(30:1095, n(), replace = TRUE), # 1 mois à 3 ans
            IMB_ALD_NUM = sample(
                c(1, 3, 5, 8, 12, 13, 30),
                n(),
                replace = TRUE
            ),
            MED_MTF_COD = sample(cv_codes, n(), replace = TRUE),
            IMB_ETM_NAT = sample(
                c("01", "02", "03", "11", "12", "13"),
                n(),
                replace = TRUE,
...
create_mock_mco_tables <- function(
    ...
    # Codes diagnostics cardiovasculaires
    cv_dp_codes <- c("I10", "I11", "I20", "I21", "I25", "I50", "I80")
    cv_dr_codes <- c("I10", "I11", "I12", "I48", "I70", "I80", "E78")
    cv_da_codes <- c("E78", "E11", "I48", "I70", "N18", "Z95")
...
create_mock_er_ete_f <- function(er_prs_f) {
    ...
    sampled_rows <- sample_n(er_prs_f, size = sample_size)
    ...
    sampled_rows %>%
        ...
        mutate(
            ETE_NUM = sample(1000:9999, n(), replace = TRUE),
            ETE_IND_TAA = sample(
                c(0, 1, 10),
                n(),
                replace = TRUE,
...
setup_mock_database <- function(
    ...
    message(paste("Tables MCO pour l'année", year))
