...
test_that("extract_ij_erprsf_works ", {
  ...
  fake_erprsf <- tibble::tribble(
    ~BEN_NIR_PSA , ~BEN_RNG_GEM , ~EXE_SOI_DTD          , ~FLX_DIS_DTD          , ~FLX_TRT_DTD          , ~PSE_SPE_COD , ~PFS_EXE_NUM , ~PRS_NAT_REF , ~PRS_ACT_NBR , ~PRS_ACT_QTE , ~EXE_SOI_DTF          , ~DPN_QLF , ~PRS_DPN_QLP , ~CPL_MAJ_TOP , ~CPL_AFF_COD , ~PSE_STJ_COD , ~RGO_ASU_NAT , ~BSE_REM_MNT , ~IJR_EMP_NUM      , ~ORL_BSE_NUM , ~PRS_FAC_TOP , ~ORG_AFF_BEN , ~RGM_COD ,
  ...
  expect_equal(
    ...
    tibble::tribble(
      ~BEN_IDT_ANO , ~BEN_RNG_GEM , ~EXE_SOI_DTD          , ~EXE_SOI_DTF          , ~PRS_ACT_NBR , ~BSE_REM_MNT , ~PFS_EXE_NUM , ~PRS_NAT_REF , ~RGO_ASU_NAT , ~IJR_EMP_NUM      ,
                 1 ,            1 , as.Date("2019-01-10") , as.Date("2019-01-10") ,            1 ,           40 ,            1 , "6110"       ,           10 , "077664451202199" ,
                 2 ,            1 , as.Date("2019-01-02") , as.Date("2019-01-06") ,            5 ,          200 ,            2 , "6120"       ,           10 , "077664451202199" ,
                 3 ,            1 , as.Date("2019-01-03") , as.Date("2019-01-12") ,           10 ,          400 ,            3 , "6110"       ,           40 , "077664451202199"
    )
  )
  #nolint end
})
