...
extract_drug_erprsf <- function(
  ...
  if (!is.null(patients_ids_filter)) {
    ...
    if (DBI::dbExistsTable(conn, TNAME_FILTER_PATIENTS)) {
      DBI::dbExecute(
        conn,
  ...
  # reset default parallelism level
  if (!is.null(r_cluster_cores) && r_cluster_cores > 1L) {
    DBI::dbExecute(conn, "ALTER SESSION SET parallel_degree_limit = AUTO")
  }
  result
...
#' \code{extract_drug_erprsf} via
#' \code{parallelize_query_by_flx_month} pour construire et exécuter la
