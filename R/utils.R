...
#' @examples
#' \dontrun{
#' # Requête de prescriptions de médicament par mois de flux en parallèle sur 4
#' sessions R.
...
parallelize_query_by_flx_month <- function(
  ...
  if (!is.null(r_cluster_cores) && r_cluster_cores > 1) {
    ...
    logger::log_info("Processing first month to create the table")
    first_month_params <- months_to_process[[1]]
    first_month_params$conn <- conn
    query_builder_function(first_month_params)

    logger::log_info("Processing remaining months in parallel")
