...
Dans le cadre de cette étude "exemple", nous souhaitons analyser les parcours de soins des patients hospitalisés pour un AVC au cours de l'année 2024. Nous utiliserons les fonctions disponibles dans `sndsTools` pour extraire et analyser les données pertinentes.
...
1. Identifier les patients avec une hospitalisation pour AVC en 2024 (codes CIM-10 : I61, I62, I63, I64) via la fonction `extract_stays_mcob()`
...
4. Extraire leurs Affections de Longue Durée (ALD) via `extract_longtermdiseases_irimbr()`

5. Extraire leurs prescriptions médicamenteuses en ville
    via `extract_drugs_erphaf()`

6. Fermer la connexion à la base de données

## Prérequis

Si vous souhaitez reproduire cette étude sur le portail CNAM, assurez-vous d'avoir :
...
Nous commençons par extraire les séjours hospitaliers avec un diagnostic principal d'AVC en utilisant la fonction `extract_stays_mcob()`.
...
Nous extrayons les ALD des patients avec AVC en utilisant `extract_longtermdiseases_irimbr()`.
...
Nous extrayons les délivrances de médicaments des patients avec AVC en utilisant `extract_drugs_erphaf()`.
